...
数组的静态方法 Array.from() 以及实例方法 concat() 和 slice()，都可以实现数组的浅拷贝，但是也局限于只能用于数组拷贝。
...
V8 引擎的垃圾回收机制采用**分代式垃圾回收机制**，其根据**对象的存活时间**将内存的垃圾回收进行不同的分代，然后对不同的分代采取不同的垃圾回收算法。

### 新生代

新生代(new_space)主要用于存放存活时间较短的对象，
